...
if ($report_id) {
    $stmt = $pdo->prepare("SELECT * FROM engelli_rampasi_reports WHERE id = ? AND kurum_id = ?");
    $stmt->execute([$report_id, $kurum_id]);
    $report = $stmt->fetch();
    if (!$report) {
        redirect('engelli_rampasi_kontrol.php'); // Invalid ID
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($report_id) {
            // Update
            $sql = "UPDATE engelli_rampasi_reports SET 
                report_date=?, start_date=?, end_date=?, next_control_date=?, isg_katip_id=?,
                firma_adi_eki=?, control_reason=?, report_text=?, result=?, authorized_person_id=?
            ...
            $stmt->execute([$kurum_id]);
            $k_codes = $stmt->fetch();
            $report_no = $k_codes['il_kodu'] . '-' . $k_codes['kurum_kodu'] . '-ramp-' . time();

            $sql = "INSERT INTO engelli_rampasi_reports 
        ...
        redirect("../results/engelli_rampasi_sonuclar.php?report_id=$report_id&status=success");
...
$default_text = htmlspecialchars($kurum['firma_adi'] ?? 'Kurum') . " yetkililerinin müracatları ile kontrolü yapılan yurt girişindeki engelliler ve hareket kısıklığı bulunan kişiler için mevcut olan rampanın standartlara uygun ve güvenli olduğu tarafımızdan görülmüştür.";

include '../../includes/header.php';
?>

<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2"><?php echo $report_id ? 'Rapor Düzenle' : 'Yeni Rapor Oluştur'; ?>: Engelli Rampası Güvenliği</h1>
            ...
            <div id="c1" class="accordion-collapse collapse show" data-bs-parent="#accordionForm">
                <div class="accordion-body">
                    <div class="row mb-3">
                        ...
                        <div class="col-md-4">
                            <label class="form-label fw-bold">Firma Adı Eki (Bölüm/Şube vb.)</label>
                            <input type="text" class="form-control" name="firma_adi_eki" id="firma_adi_eki"
                                value="<?php echo htmlspecialchars($report['firma_adi_eki'] ?? ''); ?>" placeholder="Örn: Giriş Kapısı Rampa">
                        </div>
                        <div class="col-md-4">
