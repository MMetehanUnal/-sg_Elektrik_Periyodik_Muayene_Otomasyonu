...
if ($report_id) {
    $stmt = $pdo->prepare("SELECT * FROM isinma_tesisat_reports WHERE id = ? AND kurum_id = ?");
    $stmt->execute([$report_id, $kurum_id]);
    $report = $stmt->fetch();
    if (!$report) {
        redirect('isinma_tesisat_kontrol.php'); // Invalid ID
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Heating specific header fields
    ...
    try {
        if ($report_id) {
            // Update Report
            $sql = "UPDATE isinma_tesisat_reports SET 
            ...
            $sql = "INSERT INTO isinma_tesisat_reports 
        ...
        redirect("../results/isinma_tesisat_sonuclar.php?report_id=$report_id&status=success");
...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2"><?php echo $report_id ? 'Rapor Düzenle' : 'Yeni Rapor Oluştur'; ?>: Isınma Tesisatı Güvenliği</h1>
