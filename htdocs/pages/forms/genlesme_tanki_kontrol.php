...
if ($report_id) {
    $stmt = $pdo->prepare("SELECT * FROM genlesme_tanki_reports WHERE id = ? AND kurum_id = ?");
    $stmt->execute([$report_id, $kurum_id]);
    $report = $stmt->fetch();
    if (!$report) {
        redirect('genlesme_tanki_kontrol.php'); // Invalid ID
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($report_id) {
            // Update
            $sql = "UPDATE genlesme_tanki_reports SET 
            ...
            $stmt->execute([$kurum_id]);
            $k_codes = $stmt->fetch();
            $report_no = $k_codes['il_kodu'] . '-' . $k_codes['kurum_kodu'] . '-gtank-' . time();

            $sql = "INSERT INTO genlesme_tanki_reports 
        ...
        redirect("../results/genlesme_tanki_sonuclar.php?report_id=$report_id&status=success");
...
$default_hydro = "Genleşme Tankının bütün bağlantıları kapatıldı. Tank 20 °C su ile " . (!empty($report['test_pressure']) ? htmlspecialchars($report['test_pressure']) : "12") . " Bar basınç altında 1/2 saat bekletildi. Genleşme Tankında deformasyon ve sızıntıların olmadığı görüldü.";
$default_res_text = "Yukarıda teknik özellikleri verilen ve kontrol tarihinde durumu belirtilen Genleşme Tankının testi " . (!empty($report['test_pressure']) ? htmlspecialchars($report['test_pressure']) : "12") . " Bar basınç altında yapılmış olup bir sonraki kontrol tarihine kadar kullanılmasında İŞÇİ SAĞLIĞI ve İŞ GÜVENLİĞİ açısından risk taşımamaktadır. İşletmesi UYGUNDUR.";

include '../../includes/header.php';
?>

<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2"><?php echo $report_id ? 'Rapor Düzenle' : 'Yeni Rapor Oluştur'; ?>: Genleşme Tankı Güvenliği</h1>
...
document.getElementById('test_pressure').addEventListener('input', function() {
    ...
    hydroBox.value = "Genleşme Tankının bütün bağlantıları kapatıldı. Tank 20 °C su ile " + val + " Bar basınç altında 1/2 saat bekletildi. Genleşme Tankında deformasyon ve sızıntıların olmadığı görüldü.";
    resBox.value = "Yukarıda teknik özellikleri verilen ve kontrol tarihinde durumu belirtilen Genleşme Tankının testi " + val + " Bar basınç altında yapılmış olup bir sonraki kontrol tarihine kadar kullanılmasında İŞÇİ SAĞLIĞI ve İŞ GÜVENLİĞİ açısından risk taşımamaktadır. İşletmesi UYGUNDUR.";
});
</script>
