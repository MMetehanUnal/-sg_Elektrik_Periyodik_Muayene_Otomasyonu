...
if ($report_id) {
    $stmt = $pdo->prepare("SELECT * FROM sihhi_tesisat_reports WHERE id = ? AND kurum_id = ?");
    $stmt->execute([$report_id, $kurum_id]);
    $report = $stmt->fetch();
    if (!$report) {
        redirect('sihhi_tesisat_kontrol.php'); // Invalid ID
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Sihhi tesisat specific header fields
    ...
    try {
        if ($report_id) {
            // Update Report
            $sql = "UPDATE sihhi_tesisat_reports SET 
            ...
            $stmt->execute([$kurum_id]);
            $k_codes = $stmt->fetch();
            $report_no = $k_codes['il_kodu'] . '-' . $k_codes['kurum_kodu'] . '-st-' . time();

            $sql = "INSERT INTO sihhi_tesisat_reports 
        ...
        redirect("../results/sihhi_tesisat_sonuclar.php?report_id=$report_id&status=success");
...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2"><?php echo $report_id ? 'Rapor Düzenle' : 'Yeni Rapor Oluştur'; ?>: Sıhhi Tesisat Güvenliği</h1>
