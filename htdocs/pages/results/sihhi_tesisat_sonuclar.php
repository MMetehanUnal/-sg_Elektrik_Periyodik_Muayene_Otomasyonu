...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    ...
    $stmt = $pdo->prepare("DELETE FROM sihhi_tesisat_reports WHERE id = ? AND kurum_id = ?");
    $stmt->execute([$delete_id, $kurum_id]);
    redirect('/pages/results/sihhi_tesisat_sonuclar.php');
}
...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2">Sıhhi Tesisat: Tespit ve Değerlendirmeler</h1>
...
<div class="card shadow-sm">
    <div class="card-header bg-light d-flex justify-content-between align-items-center">
        ...
        <a href="/pages/forms/sihhi_tesisat_kontrol.php" class="btn btn-sm btn-primary">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-hover align-middle">
                ...
                <tbody>
                    <?php
                    $stmt = $pdo->prepare("SELECT id, report_no, report_date, firma_adi_eki, kurum_yoneticisi, result FROM sihhi_tesisat_reports WHERE kurum_id = ? ORDER BY report_date DESC");
                    ...
                    while ($row = $stmt->fetch()):
                        ...
                        <tr class="<?php echo $is_highlighted ? 'table-warning' : ''; ?>">
                            ...
                            <td class="text-center">
                                <div class="btn-group">
                                    <a href="/pages/forms/sihhi_tesisat_kontrol.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-primary">
                                        <i class="fas fa-edit me-1"></i> Düzenle
                                    </a>
                                    <a href="/pages/sihhi_tesisat_yazdir.php?id=<?php echo $row['id']; ?>" target="_blank" class="btn btn-sm btn-dark">
                    ...
                    <?php if ($stmt->rowCount() == 0): ?>
                        <tr>
                            <td colspan="6" class="text-center py-4 text-muted">Henüz sıhhi tesisat raporu oluşturulmamış.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
